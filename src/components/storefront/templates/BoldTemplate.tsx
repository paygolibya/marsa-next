...
export default function BoldTemplate({
  ...
  // The stats banner used to be a full-bleed strip between the hero and
  // <main> — moved into a rounded card inside <main> so it can genuinely
  // take part in the drag-and-drop section order like the other templates,
  // while keeping Bold's high-contrast "banner" identity (dark bg, white
  // text) rather than just becoming plain body text.
  const statsSection =
  ...
  const newsletterSection = store.customization?.showNewsletter ? (
    <div className="rounded-3xl p-10 text-center max-w-xl mx-auto text-white shadow-2xl" style={{ backgroundColor: primary }}>
